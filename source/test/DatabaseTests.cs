...
public class DatabaseTests : TestBase
{
    ...
    {
        var db = new Database(GetCorpusDatabase("test"));
    ...
    public void Write()
    {
        var db = LoadDatabase("test-write");
        db.AddFile(GetCorpusFile("simple", "1"), GetCorpusDirectory("simple"), PathType.RelativePath, ChecksumType.MD5);
        Assert.IsTrue(db.HasChanges());
        db.Write();
    ...
    {
        var db = LoadDatabase("test-write", pathType: ChecksumVerifier.PathType.FullPath);
    ...
    {
        var db = LoadDatabase("test-write", pathType: ChecksumVerifier.PathType.FullPathNoDrive);
        db.AddFile(GetCorpusFile("simple", "1"), GetCorpusDirectory("simple"), PathType.FullPathNoDrive, ChecksumType.MD5);

        Assert.IsTrue(db.HasFile(GetCorpusFile("simple", "1")[3..]));
